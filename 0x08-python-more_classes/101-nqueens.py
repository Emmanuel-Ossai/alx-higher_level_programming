...
This determines all possible solutions to placing N non-attacking
...
where `r` and `c` represent the row and column, respectively, where a(chessboard)):
            if chessboard[r][c] == "Q":
                solution.append([r, c])
                break
    return solution
...
def mark_unavailable_spots(chessboard, row, col):
    """Mark the spots on the chessboard where non-attacking queens cannot be placed.
...
def get_solution(chessboard):
    ...
    for r in range(len(chessboard)):
        for c in range(len   # Mark all backward spots
...
def solve_nqueens(chessboard, row, queens, solutions):
    ...
    for col in range(n):
        if chessboard[row][col] == " ":
            ...
            solutions = solve_nqueens(temp_chessboard, row + 1, queens + 1, solutions)
